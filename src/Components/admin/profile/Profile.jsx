...
export default function Profile() {
  ...
  const validatePassword = () => {
    if (!passwordData.oldPassword || !passwordData.newPassword || !passwordData.confirmPassword)
      return (setPasswordError('All fields are required'), false)
    if (passwordData.newPassword.length < 6)
      return (setPasswordError('New password must be at least 6 characters'), false)
    if (passwordData.newPassword !== passwordData.confirmPassword)
      return (setPasswordError('New passwords do not match'), false)
    return true
  }
  ...
  const handleFileSelect = e => {
    ...
    if (!['image/jpeg', 'image/jpg', 'image/png', 'image/gif'].includes(file.type))
      return setPhotoError('Please select a valid image (JPEG, PNG, or GIF)')
    if (file.size > 5 * 1024 * 1024)
      return setPhotoError('File size must be less than 5MB')
    setSelectedFile(file); setPhotoError('')
    if (previewUrl) URL.revokeObjectURL(previewUrl)
    setPreviewUrl(URL.createObjectURL(file))
  }

  const handleUploadPhoto = async () => {
    if (!selectedFile) return setPhotoError('Please select a file to upload')
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setTimeout(() => {
          setShowPhotoModal(false); setPhotoSuccess('')
  ...
  const handleCancelPhoto = () => {
    setShowPhotoModal(false); setSelectedFile(null)
    setPreviewUrl(''); setPhotoError(''); setPhotoSuccess('')
    if (fileInputRef.current) fileInputRef.current.value = ''
    if (previewUrl) URL.revokeObjectURL(previewUrl)
  }
  ...
  /* ── Loading ── */
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-4 bg-neutral-50" style={{ fontFamily: 'var(--font-sans)' }}>
  ...
  if (error) return (
    <div className="flex items-center justify-center min-h-screen bg-neutral-50" style={{ fontFamily: 'var(--font-sans)' }}>
    ...
    >
          ...
          <div className="px-6 sm:px-8 pb-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-5 -mt-14 mb-8">
              ...
              <div className="flex-1 pb-1">
                ...
                <p className="text-base text-neutral-600 capitalize mb-3">{profile.designation}</p>
                
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-50 text-primary-700 text-xs font-semibold border border-primary-200">
                    <BadgeIcon />
                    {profile.employeeCode}
                  </span>
                  <span className="inline-flex items-center px-3 py-1.5 rounded-lg bg-neutral-100 text-neutral-700 text-xs font-semibold border border-neutral-200">
                    {profile.role}
                  </span>
                </div>
              </div>
            ...
            {/* Info grid - now showing ALL fields */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InfoCard icon={<MailIcon />}     label="Email Address"  value={profile.email} />
              <InfoCard icon={<PhoneIcon />}    label="Phone Number"   value={profile.mobileNumber} />
              ...
              {profile.department && (
                <InfoCard
                  ...
                  sub={`Code: ${profile.department.departmentCode}`}
                />
              )}
              
              <InfoCard icon={<UserIcon />} label="Designation" value={profile.designation} />
              
              {/* Additional info cards for completeness */}
              <InfoCard icon={<BadgeIcon />} label="Employee Code" value={profile.employeeCode} />
              <InfoCard icon={<UserIcon />} label="Role" value={profile.role} />
            </div>

            {/* Quick stats */}
            <div className="mt-6 grid grid-cols-3 gap-3">
              <div className="bg-primary-50 rounded-xl p-3 text-center border border-primary-100">
                <p className="text-xs text-primary-600 font-medium">Employee ID</p>
                <p className="text-sm font-bold text-primary-700 mt-1">{profile.employeeCode}</p>
              </div>
              <div className="bg-primary-50 rounded-xl p-3 text-center border border-primary-100">
                <p className="text-xs text-primary-600 font-medium">Department</p>
                <p className="text-sm font-bold text-primary-700 mt-1 truncate">
                  {profile.department?.departmentCode || 'N/A'}
                </p>
              </div>
              <div className="bg-primary-50 rounded-xl p-3 text-center border border-primary-100">
                <p className="text-xs text-primary-600 font-medium">Status</p>
                <p className={`text-sm font-bold mt-1 ${sc.cls.split(' ')[1]}`}>{profile.status}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showPasswordModal && (
        <Modal
          ...
          icon={<LockIcon />}
          onClose={() => {
            setShowPasswordModal(false); setPasswordError(''); setPasswordSuccess('')
            setPasswordData({ oldPassword: '', newPassword: '', confirmPassword: '' })
          }}
          size="md"
        >
        ...
        >
          <div className="flex flex-col gap-6">
            ...
            >
              ...
              <div>
                <p className="text-base font-semibold text-neutral-800">Click to select a photo</p>
