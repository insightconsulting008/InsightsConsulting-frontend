...
import {
  ArrowLeft, Plus, Download, Users, Shield, UserCheck, Grid3x3,
  Search, Filter, ArrowUpDown, MoreVertical, ExternalLink, Edit2,
  EyeOff, Trash2, RefreshCw
} from 'lucide-react';
...
const EmployeeManagement = () => {
  ...
  const fetchEmployees = useCallback(async (pageArg = page, limit = pageSize) => {
    ...
    try {
      ...
      setEmployees(list);
      setServerTotalPages(pagination?.totalPages ?? Math.max(1, Math.ceil((pagination?.totalEmployees ?? list.length) / limit)));
  ...
  const fetchDepartments = useCallback(async (pageArg = deptPage, limit = deptPageSize) => {
    ...
    try {
      ...
      setDepartments(list);
      setDeptServerTotalPages(pagination?.totalPages ?? Math.max(1, Math.ceil((pagination?.total ?? list.length) / limit)));
  ...
  useEffect(() => {
    if (activeTab === 'departments') fetchDepartments(deptPage);
  }, [activeTab, deptPage, fetchDepartments]);
  ...
  useEffect(() => {
    if (deptServerTotalPages && deptPage > deptServerTotalPages) setDeptPage(deptServerTotalPages);
  ...
  const filteredEmployees = useMemo(() => {
    ...
    if (employeeSort === 'name')       list.sort((a, b) => ((a.name || '').localeCompare(b.name || '')));
      ...
      list.sort((a, b) => (deptMap[a.departmentId] || '').localeCompare(deptMap[b.departmentId] || ''));
    }
    return list;
  ...
  const confirmDeleteDept = async () => {
    ...
    try {
      ...
      setDeptDeleteError(err?.response?.data?.message ?? 'Delete failed');
    } finally {
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search employees (name, ID, email, phone, team)"
                value={searchQuery}
            ...
            <div className="flex items-center gap-2">
              <select value={employeeFilter} onChange={(e) => { setEmployeeFilter(e.target.value); setPage(1); }} className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-primary text-sm">
                <option value="All">All Status</option>
                <option value="Active">Active Only</option>
                <option value="Inactive">Inactive Only</option>
              </select>
              <select value={employeeSort} onChange={(e) => { setEmployeeSort(e.target.value); setPage(1); }} className="px-3 py-2 border border-gray-300 focus:outline-none focus:border-primary rounded-lg text-sm">
          ...
          <div className="bg-white rounded-xl border-2 border-gray-200">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-primary-50">
                    {['Employee Name','Email Address','Phone Number','Team','DOJ','Employee Status','Actions'].map((header, index) => (
                      <th key={header} className={`px-3 py-3 text-center text-xs font-medium text-primary-800 uppercase tracking-wider ${index === 0 ? "rounded-tl-xl" : ""} ${index === 6 ? "rounded-tr-xl" : ""}`}>
                        {header}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {loadingEmployees ? (
                    ...
                    <tr><td colSpan={7} className="px-6 py-8"><div className="flex flex-col items-center justify-center text-center"><h3 className="text-base font-semibold text-gray-900 mb-1">Error</h3><p className="text-xs text-gray-500">{error}</p></div></td></tr>
                  ) : pageItems.length === 0 ? (
                    <tr><td colSpan={7} className="px-3 py-20"><div className="flex flex-col items-center justify-center text-center"><img src="https://ik.imagekit.io/vqdzxla6k/insights%20consultancy%20/Frame%202147225812.png" className='h-32' alt="" /><h3 className="text-base font-semibold text-gray-900 mb-1 mt-4">No employee records found</h3><p className="text-xs text-gray-500">It looks like there are no employee details available yet.</p></div></td></tr>
                  ) : (
                    pageItems.map((emp) => (
                      <tr key={emp.employeeId || emp.email || emp._id} className="hover:bg-primary-50 transition-colors">
                        ...
                        <td className="px-3 py-4 border border-gray-200 text-center">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${emp.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'}`}>
                            {emp.status || '—'}
                          </span>
                        </td>
                        <td className="px-3 py-4 border border-gray-200 text-sm text-gray-700 text-center">
              ...
              <div className="border-t border-gray-200 px-4 md:px-6 py-4 hidden md:flex flex-col sm:flex-row items-center justify-center gap-2">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="w-full sm:w-auto border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-primary-50 hover:border-primary hover:text-primary rounded-lg transition-colors disabled:opacity-50">Previous</button>
                <span className="px-4 py-2 text-sm text-gray-700">Page {page} of {totalPages}</span>
                <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="w-full sm:w-auto border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-primary-50 hover:border-primary hover:text-primary rounded-lg transition-colors disabled:opacity-50">Next</button>
              </div>
              <div className="border-t border-gray-200 md:hidden px-4 py-4 flex items-center justify-center gap-2">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="w-full border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 flex justify-center items-center hover:bg-primary-50 rounded-lg transition-colors disabled:opacity-50"><FaChevronLeft /></button>
                <span className="px-4 py-2 text-sm w-full text-gray-700">{page} of {totalPages}</span>
                <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="w-full border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 flex justify-center items-center hover:bg-primary-50 rounded-lg transition-colors disabled:opacity-50"><FaChevronRight /></button>
              </div>
            </div>
          </div>
        </>
      ...
      <div>
        <div className="flex py-6 flex-col sm:flex-row items-center gap-3">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
            <input type="text" placeholder="Search departments (name, code, color)" value={deptSearchQuery} onChange={(e) => { setDeptSearchQuery(e.target.value); setDeptPage(1); }}
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm md:text-base" />
          </div>
          <div className="flex items-center gap-2">
            <select value={deptFilter} onChange={(e) => { setDeptFilter(e.target.value); setDeptPage(1); }} className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-primary text-sm">
              <option>All</option>
              <option>With Employees</option>
              <option>No Employees</option>
            </select>
            <select value={deptSort} onChange={(e) => setDeptSort(e.target.value)} className="px-3 py-2 border border-gray-300 focus:outline-none focus:border-primary rounded-lg text-sm">
              <option value="name">Sort: Name</option>
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>
                {loadingDepartments ? (
                  ...
                  <tr><td colSpan={7} className="px-6 py-8"><div className="flex flex-col items-center justify-center text-center"><h3 className="text-base font-semibold text-gray-900 mb-1">Error</h3><p className="text-xs text-gray-500">{deptError}</p></div></td></tr>
          ...
          <div className="border-t md:hidden border-gray-200 px-4 py-4 flex items-center justify-center gap-2">
            <button onClick={() => setDeptPage((p) => Math.max(1, p - 1))} disabled={deptPage <= 1} className="w-full border sm:w-auto px-4 py-2 text-sm font-medium text-gray-700 flex items-center justify-center hover:bg-primary-50 rounded-lg transition-colors disabled:opacity-50"><FaChevronLeft /></button>
            <span className="px-4 py-2 text-sm w-full text-center text-gray-700">{deptPage} of {deptTotalPages}</span>
            <button onClick={() => setDeptPage((p) => Math.min(deptTotalPages, p + 1))} disabled={deptPage >= deptTotalPages} className="w-full border sm:w-auto px-4 py-2 text-sm font-medium text-gray-700 flex items-center justify-center hover:bg-primary-50 rounded-lg transition-colors disabled:opacity-50"><FaChevronRight /></button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className='bg-white py-6 border-b border-gray-200'>
        <div className="flex flex-col md:flex-row items-start md:items-center max-w-7xl mx-auto justify-between gap-4 px-6">
          <div className="flex items-center gap-3">
            <button className="p-2 hover:bg-primary-50 rounded-lg transition-colors"><ArrowLeft className="w-5 h-5 text-primary" /></button>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {activeTab === 'employees' ? (
              <button
                ...
                className="flex items-center gap-2 bg-primary text-white px-4 py-3 rounded-lg hover:bg-primary-hover transition-colors text-sm md:text-base whitespace-nowrap"
              >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 md:px-4 py-3 border border-gray-300 rounded-lg hover:bg-primary-50 hover:border-primary hover:text-primary transition-colors text-sm md:text-base whitespace-nowrap"
            >
            ...
            <button
              ...
              className={`px-4 md:px-6 py-2.5 rounded-lg font-medium transition-colors whitespace-nowrap text-sm md:text-base ${
                activeTab === t.key
                  ...
                  : 'bg-gray-100 text-gray-700 hover:bg-primary-50 hover:text-primary border border-gray-200'
              }`}
            >
              {t.label}
            </button>
      ...
      {showDeleteConfirm && employeeToDelete && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md relative" style={{ boxShadow: 'var(--shadow-2xl)' }}>
            <div className="flex items-center bg-primary rounded-t-xl gap-3 p-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center overflow-hidden">
                {employeeToDelete?.photoUrl ? (
                  ...
                  <Users className="bg-white rounded-full p-1 text-primary" />
                )}
              </div>
            ...
            <button
              onClick={() => { setShowDeleteConfirm(false); setEmployeeToDelete(null); setDeleteSuccess(false); }}
              ...
              {deleteSuccess ? (
                ...
                <>
                  ...
                  <div className="flex w-[60%] mx-auto gap-3">
                    <button onClick={confirmDelete} disabled={deleting} className="flex-1 bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary-hover transition-colors disabled:opacity-60">
                      {deleting ? 'Deleting...' : 'Confirm'}
                    </button>
                    <button onClick={cancelDelete} disabled={deleting} className="flex-1 border px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">
                      Cancel
                    </button>
                  </div>
                </>
      ...
      {showDeptDeleteConfirm && deptToDelete && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md relative">
            <button
              onClick={() => { setShowDeptDeleteConfirm(false); setDeptToDelete(null); setDeptDeleteSuccess(false); }}
            ...
            <div className="p-6">
              {deptDeleteSuccess ? (
                ...
                <>
                  ...
                  <div className="flex w-[60%] mx-auto gap-3">
                    <button onClick={confirmDeleteDept} disabled={deletingDept} className="flex-1 bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary-hover transition-colors disabled:opacity-60">
                      {deletingDept ? 'Deleting...' : 'Confirm'}
                    </button>
                    <button onClick={cancelDeleteDept} disabled={deletingDept} className="flex-1 border px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">
                      Cancel
                    </button>
                  </div>
                </>
