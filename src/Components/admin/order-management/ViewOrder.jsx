...
import { 
  ArrowLeft, Package, Calendar, DollarSign, User, Building, Mail, Phone,
  FileText, CheckCircle, Clock, AlertCircle, Download, Eye, RefreshCw,
  Layers, ShieldCheck, Award, FileBarChart, Check, UserCheck, Info,
  ExternalLink, ChevronRight, ChevronDown, ChevronUp, Image as ImageIcon,
  File, Hash, Type, Radio, CheckSquare, CalendarDays, MapPin, Globe,
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-1">Size: {(parseFloat(value.sizeInMb) || 0).toFixed(2)} MB</p>
            </div>
            <a href={value.url} download className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Download">
      ...
      case 'phone':
        return (
          <div className="flex items-center gap-2">
            ...
            <a href={`tel:${value}`} className="text-gray-700">{value}</a>
          </div>
        );
      case 'pan': case 'aadhar': case 'gst':
        return <div className="font-mono bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200">{value}</div>;
      case 'number':
        return (
          <div className="flex items-center gap-2">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-gray-500 capitalize">Type: {getFieldType(value)}</p>
                </div>
              </div>
              {expandedFields[key] ? <ChevronUp size={20} className="text-primary" /> : <ChevronDown size={20} className="text-gray-400" />}
            </button>
            {expandedFields[key] && (
            ...
            <div key={period.periodId} className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className={`px-2 py-1 rounded text-xs font-medium ${
                  ...
                  period.status === 'IN_PROGRESS' ? 'bg-primary-50 text-primary-800' :
                  'bg-amber-50 text-amber-700'
                }`}>
                  {period.status}
                </div>
              </div>
              <div className="text-xs text-gray-500">Created: {formatDate(period.createdAt)}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              {[
                ...
                { key: 'form',     label: `Form Data (${Object.keys(order.formData || {}).length})` },
                { key: 'periods',  label: `Service Periods (${order.servicePeriod?.length || 0})` },
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                  <div className="flex-shrink-0">
                    <div className="w-64 h-48 rounded-lg overflow-hidden bg-gray-100">
                      {order.service?.photoUrl ? (
                        <img src={order.service.photoUrl} alt={order.service.name} className="w-full h-full object-cover" />
                      ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className={`px-4 py-2 rounded-lg font-medium ${
                          ...
                          order.status === 'ASSIGNED'  ? 'bg-primary-50 text-primary-800' :
                          'bg-amber-50 text-amber-700'
                        }`}>
                          {order.status}
                        </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      {[
                        { icon: Calendar, label: 'Service Type', value: order.service?.serviceType },
                        { icon: RefreshCw, label: 'Frequency', value: order.service?.frequency || 'One-time' },
                        { icon: Clock, label: 'Duration', value: `${order.service?.duration} ${order.service?.durationUnit}` },
                        { icon: ShieldCheck, label: 'GST', value: order.service?.isGstApplicable === 'true' ? `${order.service?.gstPercentage || '18'}%` : 'Not Applicable' },
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="border-t pt-2 mt-2">
                          <div className="flex justify-between font-semibold">
                            ...
                            <span className="text-primary">₹{order.service?.finalIndividualPrice || '0.00'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        {activeTab === 'form' && (
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="mb-6">
              ...
              <p className="text-gray-600">Customer submitted information ({Object.keys(order.formData || {}).length} fields)</p>
            </div>
            {renderFormData()}
          </div>
        )}
        ...
        {activeTab === 'periods' && (
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="mb-6">
              ...
              <p className="text-gray-600">{order.servicePeriod?.length || 0} service periods</p>
            </div>
        ...
        <div className="mt-8">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button className="px-4 py-2 border border-primary-200 text-primary rounded-lg font-medium hover:bg-primary-50 transition-colors flex items-center gap-2">
                <RefreshCw size={16} />Reassign Order
              </button>
            </div>
          </div>
        </div>
