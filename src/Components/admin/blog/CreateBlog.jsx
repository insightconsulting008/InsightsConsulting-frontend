    ...
    <div className="fixed inset-0 bg-black/60 z-[999] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
        <div className="p-6">
          ...
          <div className="relative mb-6 bg-[var(--neutral-100)] rounded-lg overflow-hidden" style={{ height: "300px" }}>
            <Cropper
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-[var(--neutral-200)]">
            <button
              ...
              className="px-5 py-2 text-sm font-medium text-gray-700 bg-white border border-[var(--neutral-200)] rounded-lg hover:bg-[var(--neutral-50)] transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              onClick={applyCrop} disabled={cropLoading}
              className="px-5 py-2 text-sm font-medium text-white rounded-lg transition-colors disabled:opacity-60 flex items-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)]"
            >
  ...
  return (
    <div className="relative">
      <div
        ...
        className="relative bg-white border-2 border-dashed border-[var(--neutral-200)] rounded-xl transition-all duration-200 cursor-pointer overflow-hidden group hover:border-[var(--color-primary)] hover:bg-[var(--primary-50)]"
      >
        {preview ? (
          <div className="relative">
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            <div className="w-14 h-14 bg-[var(--primary-50)] rounded-full flex items-center justify-center mb-1">
              <FiImage className="w-7 h-7 text-[var(--color-primary)]" />
            </div>
...
function Panel({ title, subtitle, children, headerRight }) {
  return (
    <div className="bg-white rounded-xl border border-[var(--neutral-200)] shadow-sm overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[var(--neutral-200)] bg-[var(--neutral-50)] flex items-center justify-between">
        <div>
      ...
      <div className="bg-white w-[90%] max-w-[360px] md:max-w-[460px] rounded-2xl shadow-2xl relative">
        <div className="px-4 py-3 md:py-4 rounded-t-2xl bg-[var(--color-primary)]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex items-center justify-center">
              <FiCheck className="w-5 h-5 md:w-6 md:h-6 text-[var(--color-primary)]" />
            </div>
        ...
        <div className="px-4 md:px-6 py-6 md:py-8 text-center">
          <div className="w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 bg-[var(--primary-50)] rounded-full flex items-center justify-center">
            <FiCheck className="w-8 h-8 md:w-10 md:h-10 text-[var(--color-primary)]" />
          </div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-full gap-2 px-6 py-2.5 text-white font-medium rounded-lg transition-all bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)]"
          >
    ...
    <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
      <div className="bg-white w-[90%] max-w-[360px] md:max-w-[460px] rounded-2xl shadow-2xl relative">
        <div className="px-4 py-3 md:py-4 rounded-t-2xl bg-[var(--color-primary)]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex items-center justify-center">
              <MdErrorOutline className="w-5 h-5 md:w-6 md:h-6 text-[var(--color-primary)]" />
            </div>
        ...
        <div className="px-4 md:px-6 py-6 md:py-8 text-center">
          <div className="w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 bg-[var(--primary-50)] rounded-full flex items-center justify-center">
            <BiSolidError className="w-8 h-8 md:w-10 md:h-10 text-[var(--color-primary)]" />
          </div>
...
export default function CreateBlogPage() {
  ...
  const inputClass =
    "w-full px-3 py-2 md:py-2.5 border border-[var(--neutral-200)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-[var(--color-primary)] transition-all text-sm bg-white";
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        subtitle="Compose and publish a new blog post"
        breadcrumbs={[{ label: "Blog Management", href: "/manage-blog" }, { label: "New Post" }]}
        onBack={() => navigate("/manage-blog")}
      />

      <div className="bg-[var(--neutral-50)] min-h-screen py-6 md:py-8 px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {blockTypes.map(({ label, type, icon }) => (
                        <button
                          ...
                          className="flex items-center gap-1.5 border border-[var(--color-primary)] text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all"
                        >
                ...
                >
                  {blocks.length === 0 ? (
                    <div className="text-center py-10 rounded-xl border-2 border-dashed border-[var(--neutral-200)]">
                      <div className="w-12 h-12 rounded-xl bg-[var(--neutral-100)] flex items-center justify-center mx-auto mb-3">
                    ...
                    <div className="space-y-3">
                      {blocks.map((block, idx) => (
                        <div
                          key={block.id}
                          className="flex gap-3 items-start bg-[var(--neutral-50)] rounded-xl p-3.5 border border-[var(--neutral-200)]"
                        >
                          {/* Order controls */}
                ...
                <Panel title="Publish Settings">
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] disabled:opacity-60 text-white text-sm font-semibold py-2.5 rounded-lg transition-all shadow-sm"
                    >
                      {loading ? (
                  ...
                  <ul className="space-y-3">
                    {[
                      ...
                      <li key={tip} className="flex items-start gap-2.5 text-xs text-gray-600">
                        <span className="w-4 h-4 rounded-full bg-[var(--primary-50)] border border-[var(--primary-100)] flex items-center justify-center flex-shrink-0 mt-0.5">
                  ...
                  <ul className="space-y-2.5">
                    {[
                      ...
                      <li key={label} className="flex items-center gap-2.5 text-xs">
                        <span className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 transition-all ${done ? "bg-[var(--color-primary)] border-[var(--color-primary)]" : "bg-white border-gray-300"}`}>
