/**
 * PageHeader.jsx  —  THE single shared header for every admin page.
 *
 * Props
...
/* ── design tokens (mirrors your CSS variable system) ─────────── */
const T = {
  H: 64,
  panelW: 420,
  border: '#ebebf3',
...
const STYLE_ID = '__ph__';
function injectStyles () {
  if (typeof document === 'undefined' || document.getElementById(STYLE_ID)) return;
  const s = document.createElement('style');
  s.id = STYLE_ID;
  s.textContent = `
    @keyframes ph-down  {from{transform:translateY(-5px);opacity:0}to{transform:translateY(0);opacity:1}}
    @keyframes ph-spin  {to{transform:rotate(360deg)}}
    @keyframes ph-pulse {0%,100%{opacity:1}50%{opacity:.45}}
    @keyframes ph-bell  {
    ...
    @keyframes ph-pop{0%{transform:scale(0)}70%{transform:scale(1.3)}100%{transform:scale(1)}}
    @keyframes ph-panel-in {from{transform:translateX(100%)}to{transform:translateX(0)}}
    @media(max-width:480px){.ph-sub{display:none!important}}
    @media(max-width:380px){.ph-crumbs{display:none!important}}
    @media(max-width:767px){.ph-header{top:64px!important}}
  `;
...
function SkRow ({ i }) {
  ...
  return (
    <div style={{display:'flex',gap:12,padding:'14px 20px',borderBottom:`1px solid #f5f5fa`}}>
      <div style={{width:36,height:36,borderRadius:10,flexShrink:0,background:T.n100,animation:`ph-pulse 1.5s ${d}ms ease-in-out infinite`}}/>
      <div style={{flex:1,display:'flex',flexDirection:'column',gap:7}}>
        <div style={{height:11,width:'52%',borderRadius:6,background:T.n100,animation:`ph-pulse 1.5s ${d+80}ms ease-in-out infinite`}}/>
        <div style={{height:10,width:'82%',borderRadius:6,background:T.n100,animation:`ph-pulse 1.5s ${d+160}ms ease-in-out infinite`}}/>
        <div style={{height:10,width:'32%',borderRadius:6,background:T.n100,animation:`ph-pulse 1.5s ${d+240}ms ease-in-out infinite`}}/>
      </div>
    </div>
  );
}
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   NOTIFICATION PANEL  (private — only used inside PageHeader)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
        ...
        <div style={{
          ...
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            <Btn label="Refresh" onClick={load} disabled={loading}>
              <RefreshCw size={13} style={loading?{animation:'ph-spin .7s linear infinite'}:{}}/>
            </Btn>
            {unread > 0 && (
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,marginTop:8}}>
                    ...
                    {!n.isRead && (
                      <button onClick={()=>markOne(n.notificationId)} disabled={isBusy}
                        ...
                        {isBusy?<RefreshCw size={10} style={{animation:'ph-spin .7s linear infinite'}}/>:<CheckCheck size={10}/>}
                        Mark as read
                      </button>
                    )}
                    {n.isRead && (
...
export default function PageHeader ({ title, subtitle, breadcrumbs, onBack, onRefresh, refreshing = false, actions }) {
  ...
  return (
    <>
      <header role="banner" className="ph-header"
        style={{
          ...
          animation:'ph-down .28s ease both',
        }}>
        ...
        {onRefresh && (
          <Btn label={refreshing?'Refreshing…':'Refresh'} onClick={onRefresh} disabled={refreshing}>
            <RefreshCw size={14} style={refreshing?{animation:'ph-spin .7s linear infinite'}:{}}/>
          </Btn>
        )}
        ...
        <div style={{position:'relative',flexShrink:0}}>
          <Btn label={`Notifications${unread>0?` (${unread} unread)`:''}`} onClick={()=>setNotifOpen(v=>!v)} active={notifOpen}>
            <Bell size={15} style={bellAnim?{animation:'ph-bell .82s ease'}:{}}/>
          </Btn>
          {unread > 0 && (
            <span aria-label={`${unread} unread notifications`}
              style={{position:'absolute',top:-3,right:-3,minWidth:16,height:16,borderRadius:999,background:T.primary,color:'#fff',fontSize:unread>9?8:9,fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center',paddingInline:3,border:'2px solid #fff',boxShadow:'0 1px 4px rgba(0,0,0,.20)',pointerEvents:'none',animation:'ph-pop .3s cubic-bezier(.34,1.56,.64,1) forwards'}}>
              {unread>99?'99+':unread}
            </span>
          )}
        </div>
