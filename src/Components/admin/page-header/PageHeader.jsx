...
import {
  ...
  FileText, UserPlus, CreditCard, Edit,
} from 'lucide-react';
...
const STYLE_ID = '__aph2__';
function injectStyles() {
  if (typeof document === 'undefined' || document.getElementById(STYLE_ID)) return;
  const s = document.createElement('style');
  s.id = STYLE_ID;
  s.textContent = `
    @keyframes aph-slide { from{opacity:0;transform:translateY(-5px)} to{opacity:1;transform:translateY(0)} }
    @keyframes aph-spin   { to{transform:rotate(360deg)} }
    @keyframes aph-pulse  { 0%,100%{opacity:1} 50%{opacity:.35} }
    @keyframes aph-pop    { 0%{transform:scale(0)} 65%{transform:scale(1.3)} 100%{transform:scale(1)} }
    @keyframes aph-bell   {
...
function notifMeta(title = '') {
  ...
  if (t.includes('updat') || t.includes('amend') || t.includes('edit'))
    return { Icon: Edit,      bg: '#fefce8', iconColor: '#ca8a04' };
...
function SkRow({ delay = 0 }) {
  return (
    <div style={{ display: 'flex', gap: 12, padding: '14px 16px', borderBottom: `1px solid ${C.border}` }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: C.n100, animation: `aph-pulse 1.4s ${delay}ms ease-in-out infinite` }} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 7 }}>
        <div style={{ height: 11, width: '42%', borderRadius: 5, background: C.n100, animation: `aph-pulse 1.4s ${delay + 80}ms ease-in-out infinite` }} />
        <div style={{ height: 10, width: '72%', borderRadius: 5, background: C.n100, animation: `aph-pulse 1.4s ${delay + 160}ms ease-in-out infinite` }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {/* title + time — no bottom margin so description sits flush */}
        ...
        {/* description — 2px top gap, flush to title */}
        {notif.description && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: notif.description ? 0 : 6 }}>
          ...
          {isUnread ? (
            ...
            >
              {isBusy
                ? <RefreshCw size={10} style={{ animation: 'aph-spin .6s linear infinite' }} />
                : <CheckCheck size={10} />}
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   NOTIFICATION PANEL  (admin — uses employeeId)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <IconBtn label="Refresh" onClick={load} disabled={loading}>
              <RefreshCw size={13} style={loading ? { animation: 'aph-spin .7s linear infinite' } : {}} />
            </IconBtn>
            {unreadCount > 0 && (
              <button onClick={markAll} disabled={markingAll} style={{
                ...
                {markingAll
                  ? <RefreshCw size={10} style={{ animation: 'aph-spin .7s linear infinite' }} />
                  : <CheckCheck size={10} />}
                Mark all
              </button>
            )}
  ...
  return (
    <>
      <header role="banner" style={{
        ...
        animation: 'aph-slide .25s ease both',
      }}>
        <div style={{
          ...
          {onRefresh && (
            <IconBtn label={refreshing ? 'Refreshing…' : 'Refresh'} onClick={onRefresh} disabled={refreshing}>
              <RefreshCw size={14} style={refreshing ? { animation: 'aph-spin .7s linear infinite' } : {}} />
            </IconBtn>
          )}
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <IconBtn
              ...
              active={notifOpen}
            >
              <Bell size={15} style={bellAnim ? { animation: 'aph-bell .85s ease' } : {}} />
            </IconBtn>
            {unread > 0 && (
              <span aria-label={`${unread} unread`} style={{
                ...
                animation: 'aph-pop .3s cubic-bezier(.34,1.56,.64,1) forwards',
              }}>
                {unread > 99 ? '99+' : unread}
              </span>
            )}
