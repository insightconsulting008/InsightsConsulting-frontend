...
const STYLE_ID = '__uph__';
function injectStyles() {
  if (typeof document === 'undefined' || document.getElementById(STYLE_ID)) return;
  const s = document.createElement('style');
  s.id = STYLE_ID;
  s.textContent = `
    @keyframes uph-down  {from{transform:translateY(-5px);opacity:0}to{transform:translateY(0);opacity:1}}
    @keyframes uph-spin  {to{transform:rotate(360deg)}}
    @keyframes uph-pulse {0%,100%{opacity:1}50%{opacity:.45}}
    @keyframes uph-bell  {
    ...
    @keyframes uph-pop{0%{transform:scale(0)}70%{transform:scale(1.3)}100%{transform:scale(1)}}
    @media(max-width:480px){.uph-sub{display:none!important}}
    @media(max-width:380px){.uph-crumbs{display:none!important}}
    @media(max-width:767px){.uph-header{top:64px!important}}
  `;
...
function NIcon({ title = '', read }) {
  ...
  if (t.includes('document') || t.includes('upload'))
    return <svg width="14" height="14" fill="none" stroke={c} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>;
  if (t.includes('service') || t.includes('order'))
    return <svg width="14" height="14" fill="none" stroke={c} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z" /><path d="M16 3H8a2 2 0 00-2 2v2h12V5a2 2 0 00-2-2z" /></svg>;
...
function SkRow({ i }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '14px 20px', borderBottom: '1px solid #f5f5fa' }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: T.n100, animation: `uph-pulse 1.5s ${d}ms ease-in-out infinite` }} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 7 }}>
        <div style={{ height: 11, width: '52%', borderRadius: 6, background: T.n100, animation: `uph-pulse 1.5s ${d + 80}ms ease-in-out infinite` }} />
        <div style={{ height: 10, width: '82%', borderRadius: 6, background: T.n100, animation: `uph-pulse 1.5s ${d + 160}ms ease-in-out infinite` }} />
        <div style={{ height: 10, width: '32%', borderRadius: 6, background: T.n100, animation: `uph-pulse 1.5s ${d + 240}ms ease-in-out infinite` }} />
      </div>
    </div>
  );
}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
                    ...
                    {!n.isRead && (
                      <button onClick={() => markOne(n.notificationId)} disabled={isBusy}
                        ...
                        {isBusy ? <RefreshCw size={10} style={{ animation: 'uph-spin .7s linear infinite' }} /> : <CheckCheck size={10} />}
                        Mark as read
                      </button>
                    )}
                    {n.isRead && (
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   USER PAGE HEADER  ← the only export
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
export default function PageHeader({ title, subtitle, breadcrumbs, onBack, onRefresh, refreshing = false, actions }) {
  ...
  return (
    <>
      <header role="banner" className="uph-header"
        style={{
          ...
          animation: 'uph-down .28s ease both',
        }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {hasCrumbs && (
            <nav aria-label="Breadcrumb" className="uph-crumbs"
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <Btn label={`Notifications${unread > 0 ? ` (${unread} unread)` : ''}`} onClick={() => setNotifOpen(v => !v)} active={notifOpen}>
            <Bell size={15} style={bellAnim ? { animation: 'uph-bell .82s ease' } : {}} />
          </Btn>
          {unread > 0 && (
            <span aria-label={`${unread} unread notifications`}
              style={{ position: 'absolute', top: -3, right: -3, minWidth: 16, height: 16, borderRadius: 999, background: T.primary, color: '#fff', fontSize: unread > 9 ? 8 : 9, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingInline: 3, border: '2px solid #fff', boxShadow: '0 1px 4px rgba(0,0,0,.20)', pointerEvents: 'none', animation: 'uph-pop .3s cubic-bezier(.34,1.56,.64,1) forwards' }}>
              {unread > 99 ? '99+' : unread}
            </span>
          )}
        </div>
