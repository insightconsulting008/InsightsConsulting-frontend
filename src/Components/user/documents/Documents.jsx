...
/* ── Responsive limit: 16 on desktop, 8 on mobile ───────────────────────── */
function useLimit() {
  const [limit, setLimit] = useState(() => window.innerWidth >= 768 ? 16 : 8)
  useEffect(() => {
    const handler = () => setLimit(window.innerWidth >= 768 ? 16 : 8)
...
function FilterBar({ month, year, color, onChange, total }) {
  ...
  const labelText = isActive ? `${MONTHS[month - 1]} ${year}` : 'Filter by month'

  return (
    <div className="fb-wrap" ref={ref}>
      <button
        className={`fb-trigger ${isActive ? 'fb-trigger--active' : ''}`}
        style={isActive ? { '--fc': color } : {}}
        onClick={() => setOpen(o => !o)}
      >
...
/* ════════════════════════════════════════════════════════════════════════════
   FILE CARD  (grid view) — eye icon removed, only download remains
   ════════════════════════════════════════════════════════════════════════════ */
function FileCard({ doc, idx }) {
  ...
  return (
    <div className="fc" style={{ animationDelay: `${idx * 50}ms` }}>
      <div
        className="fc__preview"
        style={{ background: hasImg ? '#000' : 'linear-gradient(135deg,var(--neutral-100) 0%,var(--neutral-200) 100%)' }}
      >
        {hasImg
        ...
        {/* Overlay — download only, no eye */}
        {doc.fileUrl && (
          <div className="fc__overlay">
            <a href={doc.fileUrl} download className="fc__act">{Ic.dl}</a>
          </div>
        )}
...
/* ════════════════════════════════════════════════════════════════════════════
   FILE ROW  (list view) — eye icon removed, only download remains
   ════════════════════════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="fr" style={{ animationDelay: `${idx * 40}ms` }}>
      ...
      <div className="fr__actions">
        {doc.fileUrl
          ? <a href={doc.fileUrl} download className="fr__btn">{Ic.dl}</a>
...
function DetailView({ folder, onBack }) {
  const limit = useLimit()                     // 16 desktop / 8 mobile
  ...
  }, [page, month, year, folder.key, limit])
  ...
  return (
    <div className="dv">
      ...
      {/* ── active filter strip ── */}
      {month !== null && (
        <div className="filter-strip" style={{ '--fc': folder.color }}>
          ...
          <button className="filter-strip__clear" style={{ color: folder.color }}
            onClick={() => { setMonth(null); setYear(NOW.getFullYear()) }}>
            {Ic.x} Clear filter
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="empty">
          ...
          {month !== null && (
            <button className="empty__btn" onClick={() => { setMonth(null); setYear(NOW.getFullYear()) }}>
              Show all documents
            </button>
          )}
        </div>
        ...
        <div className="pager">
          ...
          <div className="pager__pages">
            {Array(totalPages).fill(0).map((_,i)=>(
              <button
                key={i}
                className={`pager__page ${page===i+1?'pager__page--on':''}`}
                onClick={()=>setPage(i+1)}
              >{i+1}</button>
            ))}
          </div>
          ...
          <span className="pager__info">
            {(page-1)*limit+1}–{Math.min(page*limit, total ?? 0)} of {total ?? 0}
          </span>
        </div>
      )}
    </div>
    ...
    <>
      ...
      <div className="docs-root">
        <style>{`
          ...
          .folder-card:hover .folder-card__glow { opacity: 1; }
          .folder-card__glow {
            position: absolute; inset: 0;
            background: radial-gradient(ellipse at 0% 0%, var(--color-primary) 0%, transparent 65%);
            opacity: 0; transition: 250ms; pointer-events: none;
            mix-blend-mode: overlay; border-radius: inherit;
          }
          .folder-card__icon { flex-shrink: 0; }
          .folder-card__body { flex: 1; min-width: 0; }
          .folder-card__label {
            font-size: 17px; font-weight: 700; color: var(--neutral-900);
            margin: 0 0 3px; letter-spacing: -0.3px;
          }
          .folder-card__desc {
            font-size: 13px; color: var(--neutral-500); margin: 0 0 11px; line-height: 1.4;
          }
          .folder-card__pill {
            display: inline-block; font-size: 11.5px; font-weight: 700;
            letter-spacing: 0.2px; color: var(--color-primary);
            background: var(--primary-50); border: 1px solid var(--color-primary);
            padding: 3px 10px; border-radius: 999px;
          }
          .folder-card__arrow {
            color: var(--neutral-400); flex-shrink: 0; transition: 250ms;
          }
          ...
          .dv { animation: slideIn 0.26s ease both; }
          ...
          .back-btn {
            ...
            box-shadow: var(--shadow-sm); transition: 200ms; font-family: inherit;
          }
          .back-btn:hover { color: var(--neutral-900); border-color: var(--neutral-400); }

          .dv__controls { display: flex; align-items: center; gap: 10px; }

          .vt {
            display: flex; background: var(--neutral-0);
            border: 1.5px solid var(--neutral-200); border-radius: 9px;
            padding: 3px; gap: 2px; box-shadow: var(--shadow-sm);
          }
          .vt__btn {
            width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;
            border: none; border-radius: 6px; background: transparent;
            color: var(--neutral-500); cursor: pointer; transition: 180ms;
          }
          .vt__btn:hover { background: var(--neutral-100); color: var(--neutral-800); }
          .vt__btn--on { background: var(--color-primary) !important; color: var(--neutral-0) !important; box-shadow: 0 1px 3px rgba(0,0,0,.1); }

          .dv__heading {
            display: flex; align-items: flex-start; justify-content: space-between;
            flex-wrap: wrap; gap: 14px; margin-bottom: 20px;
            padding-bottom: 20px; border-bottom: 1.5px solid var(--neutral-200);
          }
          .dv__heading-left { display: flex; align-items: center; gap: 16px; }
          .dv__folder-icon {
            width: 68px; height: 68px; border-radius: 18px;
            display: flex; align-items: center; justify-content: center; flex-shrink: 0;
          }
          .dv__title {
            font-size: 21px; font-weight: 800; color: var(--neutral-900);
            margin: 0 0 3px; letter-spacing: -0.4px;
          }
          .dv__sub { font-size: 13px; color: var(--neutral-500); margin: 0; }
          ...
          .fb-trigger {
            ...
            color: var(--neutral-600); background: var(--neutral-0);
            border: 1.5px solid var(--neutral-200); border-radius: 12px;
            padding: 8px 13px; cursor: pointer;
            box-shadow: var(--shadow-sm); transition: 200ms; white-space: nowrap;
          }
          .fb-trigger:hover { border-color: var(--neutral-400); color: var(--neutral-900); }
          .fb-trigger--active { background: var(--primary-50); border-color: var(--primary-100); color: var(--neutral-900); }
          .fb-trigger__cal { display: flex; color: var(--neutral-400); }
          .fb-trigger__label { flex: 1; }
          .fb-trigger__count {
            font-size: 11px; font-weight: 700;
            padding: 2px 7px; border-radius: 999px; flex-shrink: 0;
          }
          .fb-trigger__arrow { display: flex; color: var(--neutral-400); transition: 200ms; }
          .fb-trigger__clear {
            display: flex; align-items: center; justify-content: center;
            width: 18px; height: 18px; border-radius: 50%; border: none;
            background: rgba(0,0,0,.08); color: var(--neutral-600);
            cursor: pointer; padding: 0; transition: 150ms; flex-shrink: 0;
          }
          .fb-trigger__clear:hover { background: rgba(0,0,0,.15); }
          ...
          .fb-panel {
            ...
            background: var(--neutral-0); border: 1.5px solid var(--neutral-200);
            border-radius: 18px; padding: 18px; width: 272px;
            box-shadow: var(--shadow-xl); animation: dropIn 0.18s ease both;
          }
          .fb-panel__head {
          ...
          .fb-panel__reset {
            ...
            background: transparent; border: none; cursor: pointer; padding: 0; transition: 150ms;
          }
          .fb-panel__reset:hover { opacity: 0.75; }

          .fb-year-row { display: flex; gap: 5px; flex-wrap: wrap; margin-bottom: 14px; }
          .fb-year {
            font-size: 12px; font-weight: 600; color: var(--neutral-700);
            background: var(--neutral-100); border: 1.5px solid transparent;
            border-radius: 8px; padding: 5px 10px; cursor: pointer;
            transition: 150ms; font-family: inherit;
          }
          .fb-year:hover { background: var(--neutral-200); }
          .fb-year--on { background: var(--color-primary) !important; color: var(--neutral-0) !important; border-color: var(--color-primary) !important; }

          .fb-month-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 5px; }
          .fb-month {
            font-size: 12px; font-weight: 600; color: var(--neutral-700);
            background: var(--neutral-100); border: 1.5px solid transparent;
          ...
          .fb-month:hover { background: var(--neutral-200); color: var(--neutral-900); }
          .fb-month--on { background: var(--color-primary) !important; color: var(--neutral-0) !important; border-color: var(--color-primary) !important; }
          ...
          .filter-strip {
            ...
            margin-bottom: 18px; animation: fadeUp 0.2s ease both;
          }
          .filter-strip__dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
          .filter-strip__text { font-size: 13px; color: var(--neutral-600); flex: 1; min-width: 120px; }
          .filter-strip__text strong { color: var(--neutral-900); font-weight: 700; }
          .filter-strip__clear {
            display: flex; align-items: center; gap: 5px;
            font-size: 12px; font-weight: 600; background: transparent; border: none;
            cursor: pointer; transition: 150ms; flex-shrink: 0; opacity: 0.8;
            padding: 0; font-family: inherit;
          }
          .filter-strip__clear:hover { opacity: 1; }
          ...
          @media (max-width: 480px) {
            .file-grid { grid-template-columns: repeat(2,1fr); gap: 9px; }
          }
          ...
          .fc:hover { transform: translateY(-4px); box-shadow: var(--shadow-lg); border-color: var(--neutral-300); }
          .fc__preview {
            position: relative; height: 112px;
            display: flex; align-items: center; justify-content: center; overflow: hidden;
          }
          .fc__img { width: 100%; height: 100%; object-fit: cover; }
          .fc__ph { display: flex; flex-direction: column; align-items: center; gap: 6px; }
          .fc__ext {
          ...
          .fc__nofile {
            ...
            text-align: center; font-size: 9.5px; color: var(--neutral-400);
          }
          /* Overlay — centred single download button */
          .fc__overlay {
            position: absolute; inset: 0;
            background: rgba(26,26,46,0.65);
            display: flex; align-items: center; justify-content: center;
            opacity: 0; transition: 190ms; backdrop-filter: blur(3px);
          }
          .fc:hover .fc__overlay { opacity: 1; }
          .fc__act {
            width: 36px; height: 36px;
            display: flex; align-items: center; justify-content: center;
            border-radius: 50%; color: var(--neutral-0); text-decoration: none;
            background: var(--color-primary); border: 1.5px solid rgba(255,255,255,.25);
            transition: 140ms; box-shadow: 0 2px 8px rgba(0,0,0,.25);
          }
          .fc__act:hover { transform: scale(1.1); }
          .fc__meta { padding: 9px 11px; }
          .fc__name {
            font-size: 11.5px; font-weight: 600; color: var(--neutral-800);
            white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin: 0 0 2px;
          }
          .fc__date { font-size: 10px; color: var(--neutral-400); margin: 0; }

          /* ── file list ── */
          .file-list { display: flex; flex-direction: column; gap: 7px; margin-bottom: 24px; }
          .fr {
            display: flex; align-items: center; gap: 12px;
            padding: 11px 15px; background: var(--neutral-0);
            border: 1.5px solid var(--neutral-100); border-radius: 10px;
            box-shadow: var(--shadow-sm); transition: 190ms;
            animation: fadeUp 0.26s ease both;
          }
          .fr:hover { border-color: var(--neutral-300); box-shadow: var(--shadow-md); background: var(--neutral-50); }
          .fr__icon { flex-shrink: 0; }
          .fr__info { flex: 1; min-width: 0; }
          .fr__name {
            font-size: 12.5px; font-weight: 600; color: var(--neutral-800);
            margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
          }
          .fr__sub { font-size: 11px; color: var(--neutral-400); margin: 0; }
          .fr__date { font-size: 11.5px; color: var(--neutral-500); white-space: nowrap; flex-shrink: 0; }
          @media (max-width: 560px) { .fr__date { display: none; } }
          .fr__actions { display: flex; gap: 5px; flex-shrink: 0; }
          .fr__btn {
            width: 32px; height: 32px;
            display: flex; align-items: center; justify-content: center;
            border-radius: 8px; background: var(--neutral-100);
            color: var(--neutral-600); text-decoration: none; transition: 190ms;
          }
          .fr__btn:hover { background: var(--color-primary); color: var(--neutral-0); }
          .fr__dash { font-size: 16px; color: var(--neutral-300); }
          ...
          .skel--card { height: 152px; }
          .skel--row  { height: 54px; border-radius: 10px; }

          /* ── empty ── */
          .empty {
            display: flex; flex-direction: column; align-items: center;
            gap: 12px; padding: 52px 0; color: var(--neutral-400);
          }
          .empty p { font-size: 13.5px; font-weight: 500; margin: 0; color: var(--neutral-500); text-align: center; }
          .empty__btn {
            ...
            transition: 160ms; font-family: inherit;
          }
          .empty__btn:hover { background: var(--color-primary); color: var(--neutral-0); }

          /* ── pagination ── */
          .pager {
            display: flex; align-items: center; justify-content: center;
            gap: 6px; padding-top: 18px; border-top: 1.5px solid var(--neutral-100);
            flex-wrap: wrap;
          }
          .pager__btn {
            width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;
            border: 1.5px solid var(--neutral-200); border-radius: 9px;
            background: var(--neutral-0); color: var(--neutral-600); cursor: pointer;
            transition: 190ms; box-shadow: var(--shadow-sm); flex-shrink: 0;
          }
          .pager__btn:hover:not(:disabled) { border-color: var(--neutral-400); color: var(--neutral-900); }
          .pager__btn:disabled { opacity: 0.3; cursor: not-allowed; }
          .pager__pages { display: flex; gap: 3px; flex-wrap: wrap; justify-content: center; }
          .pager__page {
            width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;
            border: 1.5px solid transparent; border-radius: 9px; background: transparent;
            color: var(--neutral-600); font-size: 12.5px; font-weight: 600;
            cursor: pointer; transition: 190ms; font-family: inherit;
          }
          .pager__page:hover { background: var(--neutral-100); }
          .pager__page--on {
          ...
          .pager__info {
            font-size: 11.5px; color: var(--neutral-400); font-weight: 500;
