          ...
          <div className="grid lg:grid-cols-[1fr_420px] gap-8 lg:gap-16 items-start">
            ...
            <div>
              ...
              {selectedService?.tagline && (
                <p className="text-primary font-semibold text-[18px] lg:text-[24px] mb-5 leading-snug">
                  {selectedService.tagline}
                </p>
              )}
              ...
              {selectedService?.whyChoose && (
                <div className="mb-5 mt-5 lg:mb-8 lg:mt-8">
                  ...
                  <div className="space-y-2.5 lg:space-y-3">
                    ...
                    {selectedService.whyChoose.closingParagraphs?.map(
                      (p, i) => (
                        <p
                          key={i}
                          className="text-gray-900 text-[15px] lg:text-[17px] leading-relaxed"
              ...
              {selectedService?.heroIntro && (
                <div className="space-y-7 mb-8 pt-2 border-t border-gray-100">
                  {selectedService.heroIntro.twelveA && (
                    <HeroIntroSection
                      data={selectedService.heroIntro.twelveA}
                    />
                  )}
                  {selectedService.heroIntro.eightyG && (
                    <HeroIntroSection
                      data={selectedService.heroIntro.eightyG}
                    />
                  )}
                </div>
              )}
        ...
        {selectedService?.processSummary && (
          <div className="relative bg-primary py-5">
            <div className="container mx-auto px-4 lg:px-12 flex items-center justify-between gap-4">
              <p className="text-white font-semibold text-lg lg:text-xl leading-relaxed max-w-3xl">
                {stripLeadingEmoji(selectedService.processSummary)}
              </p>

              <button
                onClick={() => setPopupOpen(true)}
                className="flex-shrink-0 bg-white text-primary text-base sm:text-lg lg:text-xl font-bold px-6 py-2.5 rounded-xl hover:bg-border hover:text-primary transition-all duration-300 whitespace-nowrap"
              >
                Get Started
              </button>
            </div>
