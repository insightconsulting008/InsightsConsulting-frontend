...
const isHeader = (text) =>
  text &&
  !text.startsWith("👉") &&
  HEADER_STARTS.some((p) => text.startsWith(p));
...
const parseGroups = (items) => {
  ...
  items.forEach((item) => {
    if (isHeader(item)) {
      ...
      if (!current)
        current = { header: null, description: null, items: [], notes: [] };
      if (
        !current.description &&
      ...
      <div className="space-y-2.5 mt-0.5">
        {group.items.map((it, i) =>
          it.endsWith(":") ? (
            <p
              key={i}
              className="text-[11px] font-bold tracking-wider uppercase text-gray-900 mt-3 mb-0.5"
            >
            ...
            <div
              key={i}
              className="flex items-start gap-2.5 text-gray-900 text-[14px] lg:text-[15px] leading-relaxed"
            >
      ...
      <div className="space-y-2 mt-0.5">
        {group.items.map((it, i) =>
          it.endsWith(":") ? (
            <p
              key={i}
              className="text-gray-500 text-[10px] font-bold tracking-wider uppercase mt-2.5 mb-0.5"
            >
              {stripLeadingEmoji(it)}
            </p>
          ) : (
            <div
              key={i}
              className="flex items-start gap-2 text-gray-700 text-sm lg:text-[14px] leading-relaxed"
            >
        ...
        return (
          <div key={idx}>
            ...
            {subSections?.length > 0 && (
              <div
                className={`grid ${colClass(subSections.length)} gap-4 mb-4`}
              >
                {subSections.map((sub, i) => (
                  <div
                    key={i}
                    className="h-full bg-white/40 border border-primary rounded-xl p-5 lg:p-6 shadow-[0_2px_12px_rgba(124,58,237,0.06)] backdrop-blur-[1px]"
                  >
                    ...
                    {sub.items?.length > 0 && (
                      <div className="space-y-2">
                        {sub.items.map((it, j) => (
                          <div
                            key={j}
                            className="flex items-start gap-2 text-gray-900 text-[15px] lg:text-base"
                          >
            ...
            {closingParagraphs?.map((p, i) => (
              <p
                key={i}
                className="text-gray-900 text-[15px] lg:text-[17px] leading-relaxed mt-3"
              >
                {p}
              </p>
            ))}
        ...
        <div className="space-y-3 mb-4">
          {paragraphs.map((p, i) =>
            p.endsWith("?") ? (
              <div
                key={i}
                className="bg-primary/5 border-l-4 border-primary/40 rounded-r-xl px-5 py-3 my-1"
              >
        ...
        <div className="space-y-2 mb-4">
          {nestedItems.map((item, i) =>
            item.subItems?.length > 0 ? (
              ...
              <div
                key={i}
                className="flex items-start gap-2.5 text-gray-900 text-[15px] lg:text-[17px] leading-relaxed"
              >
                <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0" />
                {stripLeadingEmoji(item.text)}
              </div>
        ...
        <div className="mt-5 space-y-3">
          {closingParagraphs.map((p, i) =>
            p.startsWith("👉") ? (
              ...
              <div
                key={i}
                className="bg-primary/5 border-l-4 border-primary/40 rounded-r-xl px-5 py-3"
              >
                <p className="text-gray-900 text-[15px] lg:text-[17px] font-semibold leading-relaxed">
                  {p}
                </p>
              ...
              <p
                key={i}
                className="text-gray-900 text-[15px] lg:text-[17px] font-medium leading-relaxed"
              >
                {p}
              </p>
            ),
      ...
      {whatIs?.length > 0 && (
        <div className="grid sm:grid-cols-2 gap-5 mt-5">
          {whatIs.map((card, i) => (
            <div
              key={i}
              className="bg-primary/[0.03] border border-primary rounded-2xl p-6 lg:p-7 flex flex-col gap-3 backdrop-blur-[1px]"
            >
              ...
              <div className="space-y-3">
                {card.paragraphs?.map((p, j) => (
                  <p
                    key={j}
                    className="text-gray-900 text-[14px] lg:text-[16px] leading-relaxed"
                  >
                    {p}
                  </p>
                ))}
                      ...
                      <div key={j}>
                        ...
                        <div className="pl-5 space-y-1.5">
                          {item.subItems.map((si, k) => (
                            <div
                              key={k}
                              className="flex items-start gap-2 text-gray-900 text-[14px] lg:text-[15px]"
                            >
                              <span className="mt-[6px] w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0" />
                              {si}
                            </div>
                ...
                <div className="space-y-2">
                  {sub.items.map((it, j) =>
                    it?.isGroup ? (
                      ...
                      <div
                        key={j}
                        className="flex items-start gap-2 text-gray-900 text-[15px] lg:text-[16px] leading-relaxed"
                      >
                        <span className="mt-[8px] w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0" />
                        {stripLeadingEmoji(it)}
                      </div>
      ...
      {serviceItems?.length > 0 && (
        <div className={`grid ${colClass(serviceItems.length)} gap-4 mb-4`}>
          {serviceItems.map((item, i) => (
            <div
              key={i}
              className="h-full bg-white/40 border border-primary rounded-xl p-5 lg:p-6 shadow-[0_2px_12px_rgba(124,58,237,0.06)] hover:border-primary transition-all duration-200 backdrop-blur-[1px]"
            >
...
const GenericDarkCard = ({ section, serviceNote }) => {
  ...
  return (
    <div className="h-full bg-white/40 border-2 border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)]">
      ...
      {paragraphs?.length > 0 && (
        <div className="space-y-3 mb-5">
          {paragraphs.map((p, i) => (
            <p
              key={i}
              className="text-gray-900 text-[15px] lg:text-[16px] leading-relaxed"
            >
              {stripLeadingEmoji(p)}
            </p>
          ))}
      ...
      {groups ? (
        <div
          className={`grid ${items.filter(isHeader).length <= 2 ? "grid-cols-1" : "sm:grid-cols-2"} gap-3`}
        >
          {groups.map((g, i) => (
            <DarkItemCard key={i} group={g} />
          ))}
        </div>
      ) : (
        items?.length > 0 && (
          <ul className="space-y-3">
            {items.map((item, i) =>
              item.startsWith("👉") ||
              (item.startsWith("We ") && !item.endsWith(":")) ? (
      ...
      {closingParagraphs?.map((p, i) => (
        <p
          key={i}
          className="text-gray-900 text-[15px] lg:text-[16px] leading-relaxed mt-4"
        >
          {p}
        </p>
      ))}
      ...
      {subSections?.length > 0 && (
        <div className={`grid ${colClass(subSections.length)} gap-3 mt-4`}>
          {subSections.map((sub, i) => (
            <div
              key={i}
              className="bg-primary/[0.04] border-2 border-primary rounded-xl p-4 lg:p-5"
            >
              ...
              {sub.items?.length > 0 && (
                <ul className="space-y-1.5">
                  {sub.items.map((it, j) => (
                    <li
                      key={j}
                      className="flex items-start gap-2 text-gray-900 text-sm leading-relaxed"
                    >
                      <span className="mt-[6px] w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0" />
                      {it}
                    </li>
    ...
    <section className="py-8 lg:py-12">
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        {advantages.items.map((item, i) => (
          <div
            key={i}
            className="h-full bg-white/40 border border-primary rounded-2xl p-5 lg:p-8 shadow-[0_2px_16px_rgba(124,58,237,0.07)] hover:border-primary transition-all duration-200 backdrop-blur-[1px]"
          >
            ...
            {item.subPoints?.length > 0 && (
              <ul className="space-y-2">
                {item.subPoints.map((sp, j) => (
                  <li
                    key={j}
                    className="flex items-start gap-2 text-gray-900 text-sm lg:text-[14px]"
                  >
                    <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-gray-400 flex-shrink-0" />
...
const ServiceContent = ({ hideCta = false }) => {
  ...
  const isDocSection = (s) => {
    ...
    return (
      h.includes("document") ||
      h.includes("what we need") ||
      isImportantNotesSection(s) ||
      isSupportSection(s)
    );
  };
  const mainContentSections =
    service.contentSections?.filter((s) => !isDocSection(s)) || [];
  const docContentSections =
    service.contentSections?.filter(isDocSection) || [];
  ...
  const overviewAlone = (key) =>
    overviewKeys.length % 2 !== 0 && overviewLast === key
      ? "lg:col-span-2"
      : "";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, rgba(255,255,255,0.82) 0%, rgba(255,255,255,0.93) 40%, rgba(255,255,255,0.97) 100%)",
        }}
      />
      ...
      <div className="relative">
        ...
        ──────────────────────────────────────────────── */}
          {(service.idealFor?.length > 0 ||
            ...
            <section id="overview" className="py-5 lg:py-14">
              <div
                className={`grid gap-5 ${overviewKeys.length > 1 ? "lg:grid-cols-2" : "grid-cols-1"}`}
              >
                {service.idealFor?.length > 0 && (
                  <div
                    className={`h-full bg-white/40 border border-primary rounded-2xl p-5 lg:p-10 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${overviewAlone("idealFor")} backdrop-blur-[1px]`}
                  >
                    <CardHeading
                      text={service.idealForHeading || "Ideal For"}
                    />
                    <ul className="space-y-3">
                ...
                {service.formsHandled && (
                  <div
                    className={`h-full bg-white/40 border border-primary rounded-2xl p-5 lg:p-10 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${overviewAlone("formsHandled")} backdrop-blur-[1px]`}
                  >
                    ...
                    {service.formsHandled.note && (
                      <NoteBox text={service.formsHandled.note} />
                    )}
                  </div>
                )}
                ...
                {service.establishment && (
                  <div className={`h-full ${overviewAlone("establishment")}`}>
                    <ImportantNoteSection
                      importantNote={service.establishment}
                    />
                  </div>
                )}
              </div>
        ...
        ──────────────────────────────────────────────── */}
          {isRegistration &&
            (service.requirements?.eligibility ||
              ...
              <section id="requirements" className="py-5 lg:py-14">
                ...
                <div
                  className={`grid gap-5 ${service.requirements?.eligibility && regDocs?.length > 0 ? "sm:grid-cols-2" : "grid-cols-1"}`}
                >
                  {service.requirements?.eligibility && (
                    <div className="h-full bg-white/40 border border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)] backdrop-blur-[1px]">
                      <CardHeading
                        text={service.requirements.eligibility.heading}
                      />
                      <ul className="space-y-3">
                        {service.requirements.eligibility.items.map(
                          (item, i) => (
                            <li
                              key={i}
                              className="flex items-start gap-3 text-gray-900 text-[15px] lg:text-[17px] leading-relaxed"
          ...
          {docContentSections.length > 0 &&
            (() => {
              const regularDocSections = docContentSections.filter(
                (s) => !isImportantNotesSection(s) && !isSupportSection(s),
              );
              const importantNotesSections = docContentSections.filter(
                isImportantNotesSection,
              );
              const supportSections =
                docContentSections.filter(isSupportSection);
              const docTitle =
                regularDocSections.length === 1 && regularDocSections[0].heading
              ...
              return (
                <section id="documents" className="py-5 lg:py-14">
                  ...
                  {regularDocSections.length > 0 && (
                    <div
                      className={`grid gap-5 ${regularDocSections.length > 1 ? "sm:grid-cols-2" : "grid-cols-1"}`}
                    >
                    ...
                    <div key={`imp-${i}`} className="mt-6 space-y-1">
                      ...
                      {section.subSections?.map((sub, j) => (
                        <div key={j} className="mb-2">
                          ...
                          {sub.description && (
                            <NoteBox text={sub.description} />
                          )}
                          {sub.items?.map((pt, k) => (
                            <NoteBox key={k} text={pt} />
                          ))}
                          {sub.nestedItems?.map((ni, k) => (
                            <div key={k}>
                              {ni.text && <NoteBox text={ni.text} />}
                              {ni.subItems?.map((sp, m) => (
                                <NoteBox key={m} text={sp} />
                              ))}
                            </div>
                          ))}
                        </div>
                      ))}
                      {section.items?.map((pt, k) => (
                        <NoteBox key={k} text={pt} />
                      ))}
      ...
      ═══════════════════════════════════════════════════ */}
        {(service.insightAdvantage ||
          ...
          service.darkSections?.length > 0) && (
          <section
            id="insight"
            className="bg-[rgba(240,154,42,0.06)] backdrop-blur-[1.5px] border-t-2 border-primary py-8 lg:py-16"
          >
            <div className="container mx-auto px-4 lg:px-12">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                {service.insightAdvantage && (
                  <div
                    className={`h-full bg-white/40 border-2 border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${insightSpan("insightAdvantage")}`}
                  >
                    <div className="flex items-start gap-3 mb-4 lg:mb-6">
                      <span className="w-[3px] h-6 bg-primary rounded-full flex-shrink-0 mt-[3px]" />
                      <h3 className="text-[16px] lg:text-[20px] font-bold text-gray-900 leading-snug tracking-tight">
                ...
                {service.postSupport?.length > 0 && (
                  <div
                    className={`h-full bg-white/40 border-2 border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${insightSpan("postSupport")}`}
                  >
                    <div className="flex items-start gap-3 mb-4 lg:mb-6">
                      <span className="w-[3px] h-6 bg-primary rounded-full flex-shrink-0 mt-[3px]" />
                      <h3 className="text-[16px] lg:text-[20px] font-bold text-gray-900 leading-snug tracking-tight">
                        {stripLeadingEmoji(service.postSupportTitle) ||
                          "Post-Filing Support"}
                      </h3>
                    </div>
                    <ul className="space-y-3">
                ...
                {service.commonMistakes?.length > 0 && (
                  <div
                    className={`h-full bg-white/40 border-2 border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${insightSpan("commonMistakes")}`}
                  >
                    <div className="flex items-start gap-3 mb-4 lg:mb-6">
                      <span className="w-[3px] h-6 bg-primary rounded-full flex-shrink-0 mt-[3px]" />
                      <h3 className="text-[16px] lg:text-[20px] font-bold text-gray-900 leading-snug tracking-tight">
                ...
                {service.growthInsight && (
                  <div
                    className={`h-full bg-white/40 border-2 border-primary rounded-2xl p-5 lg:p-9 shadow-[0_2px_24px_rgba(124,58,237,0.07)] ${insightSpan("growthInsight")}`}
                  >
                    <div className="flex items-start gap-3 mb-4 lg:mb-6">
                      <span className="w-[3px] h-6 bg-primary rounded-full flex-shrink-0 mt-[3px]" />
                      <h3 className="text-[16px] lg:text-[20px] font-bold text-gray-900 leading-snug tracking-tight">
                    ...
                    {service.growthInsight.weHelpYou?.length > 0 && (
                      <>
                        {(service.growthInsight.listPrefix ??
                          "We help you:") && (
                ...
                {service.darkSections?.map((section, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`h-full${section.fullWidth ? " sm:col-span-2" : insightAlone === `dark_${i}` ? " sm:col-span-2" : ""}`}
                    >
                      <GenericDarkCard
                        section={section}
                        serviceNote={
                          isLast && !service.growthInsight
                            ? service.serviceNote
                            : undefined
                        }
                      />
